...
class App {
  ...
  #mapEvent;
  #workouts = [];
  #markers = new Map(); // workoutID => marker
  ...
  _renderWorkoutMarker(workout) {
    ...
    // Store the marker with its workout ID
  ...
  `;
    ...
    html += `
    <button class="workout__delete" title="Delete workout">🗑️</button>
    </li>
  `;

    form.insertAdjacentHTML('afterend', html);

    // Now attach delete listener
    setTimeout(() => {
      const workoutEl = document.querySelector(`[data-id="${workout.id}"]`);
      if (!workoutEl) return;
      const deleteBtn = workoutEl.querySelector('.workout__delete');
      if (!deleteBtn) return;
  ...
  _moveToPopup(e) {
    // If user clicked the delete button, don't move the map
  ...
  _getLocalStorage() {
    ...
    this.#workouts = data.map(obj => {
      let workout;
      if (obj.type === 'running') {
        workout = Object.assign(new Running(), obj);
      } else if (obj.type === 'cycling') {
        workout = Object.assign(new Cycling(), obj);
      }
      return workout;
    });
